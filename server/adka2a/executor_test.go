...
// collectEvents collects all events from an iter.Seq2[a2a.Event, error] into a slice.
// If stopAtIndex is non-nil, it stops yielding after collecting that many events (simulates consumer stopping early).
func collectEvents(seq iter.Seq2[a2a.Event, error], stopAtIndex *eventIndex) ([]a2a.Event, error) {
	...
	for event, err := range seq {
		...
		if stopAtIndex != nil && len(events) > stopAtIndex.i {
			break
		}
	}
	return events, nil
...
func TestExecutor_Execute(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "malformed data",
			request: &a2a.SendMessageRequest{Message: a2a.NewMessageForTask(a2a.MessageRoleUser, task, a2a.NewRawPart([]byte("(*_*)")))},
			wantErr: true,
		},
		{
		...
		{
			name:    "agent run fails, consumer stops early",
			request: &a2a.SendMessageRequest{Message: hiMsgForTask},
			events: []*session.Event{
				{LLMResponse: modelResponseFromParts(genai.NewPartFromText("Hello"))},
			},
			stopAtIndex:   &eventIndex{2},
			agentRunFails: fmt.Errorf("OOF"),
			wantEvents: []a2a.Event{
				a2a.NewStatusUpdateEvent(task, a2a.TaskStateWorking, nil),
				a2a.NewArtifactEvent(task, a2a.NewTextPart("Hello")),
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			events, err := collectEvents(executor.Execute(t.Context(), reqCtx), tc.stopAtIndex)
			if err != nil && !tc.wantErr {
				t.Fatalf("executor.Execute() error = %v, want nil", err)
			}
			if err == nil && tc.wantErr {
				t.Fatalf("executor.Execute() produced %d events, want error", len(events))
			}
			if tc.wantEvents != nil {
				if diff := cmp.Diff(tc.wantEvents, events, ignoreFields...); diff != "" {
					t.Fatalf("executor.Execute() wrong events (+got,-want):\ngot = %v\nwant = %v\ndiff = %s", events, tc.wantEvents, diff)
				}
			}
		})
	}
...
func TestExecutor_Cancel(t *testing.T) {
	...
	reqCtx.StoredTask = task
	events, err := collectEvents(executor.Cancel(t.Context(), reqCtx), nil)
	if err != nil {
		t.Fatalf("executor.Cancel() error = %v, want nil", err)
	}
	if len(events) != 1 {
		t.Fatalf("executor.Cancel() produced %d events, want 1", len(events))
	}
	event := events[0].(*a2a.TaskStatusUpdateEvent)
...
func TestExecutor_SessionReuse(t *testing.T) {
	...
	_, err = collectEvents(executor.Execute(ctx, reqCtx), nil)
	if err != nil {
		t.Fatalf("executor.Execute() error = %v, want nil", err)
	}
	_, err = collectEvents(executor.Execute(ctx, reqCtx), nil)
...
func TestExecutor_Callbacks(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			events, err := collectEvents(executor.Execute(t.Context(), reqCtx), nil)
...
func TestExecutor_Cancel_AfterEvent(t *testing.T) {
	...
	result, sendErr := client.SendMessage(t.Context(), &a2a.SendMessageRequest{
		Message: &a2a.Message{ID: string(msgId), Parts: []*a2a.Part{a2a.NewTextPart("TEST")}, Role: a2a.MessageRoleUser},
...
func TestExecutor_Converters(t *testing.T) {
	...
	t.Run("A2APartConverter", func(t *testing.T) {
		t.Run("modify input", func(t *testing.T) {
			...
			reqCtx := &a2asrv.ExecutorContext{TaskID: task.ID, ContextID: task.ContextID, Message: hiMsg, StoredTask: task}
			if _, err := collectEvents(executor.Execute(t.Context(), reqCtx), nil); err != nil {
	...
	t.Run("GenAIPartConverter", func(t *testing.T) {
		...
		t.Run("modify output", func(t *testing.T) {
			...
			if !found {
				t.Errorf("did not find 'WORLD' in events: %v", events)
			}
		})
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(events) != len(tc.wantEvents) {
				t.Fatalf("got %d events, want %d", len(events), len(tc.wantEvents))
			}
